...
(function () {
  ...
  // side-stepping subject (assemble archetype): track the chapter under the
  // viewport centre and read its data-align → -1 (text right), +1 (text left), 0 (centre)
  const sideOffset = STORY.archetype === 'assemble';
  let sideTarget = 1, sideCur = 1;
  if (sideOffset) {
    document.querySelectorAll('.chapter').forEach((el) => {
      ScrollTrigger.create({ trigger: el, start: 'top 50%', end: 'bottom 50%',
        onToggle: (self) => { if (self.isActive) sideTarget = el.dataset.align === 'right' ? -1 : el.dataset.align === 'center' ? 0 : 1; } });
    });
  }
  document.querySelectorAll('.chapter').forEach((el) => {
  ...
  function frame(dt) {
    ...
    if (isOrbit) {
      ...
      if (sideOffset) {
        // assemble (product) layout: the subject steps ASIDE from the copy — to the
        // right of left-aligned chapters, to the left of right-aligned ones, and
        // back (smaller) behind centred ones — so the 3D never sits under the text.
        // The target is explicit (no per-frame bias), so the lerp converges ON it.
        const reach = Math.min(1, Math.max(0.35, camera.aspect - 0.5));
        sideCur += (sideTarget - sideCur) * Math.min(1, dt * 2.5);
        const narrow = camera.aspect < 0.8 ? 20 : camera.aspect < 1.2 ? 8 : 0;
        tmpC.set(
          tmpB.x + sideCur * 11 * reach,
          tmpB.y - 2 + (camera.aspect < 0.8 ? 7 : 0), // narrow: sit ABOVE the centred copy
          tmpB.z - 16 - (1 - Math.abs(sideCur)) * 14 - narrow);
        subject.group.position.lerp(tmpC, 0.15);
        camera.lookAt(tmpB.x, tmpB.y - 2, tmpB.z - 16);
      } else {
  ...
  function buildSubject(kind, col) {
    ...
    let viewport = null;
    if (kind === 'chain') {
      // A lesional chain: nodes (structures) along a spine-like S-curve, joined by
      // glowing links. Node colour follows a severity ladder (amber → orange → red),
      // links take the accent — the same vocabulary the product's chain editor uses.
      const severity = [0xf59e0b, 0xf97316, 0xdc2626];
      const N = 9;
      const spine = new THREE.CatmullRomCurve3([
        new THREE.Vector3(0, 11, 0), new THREE.Vector3(2.2, 6, -1), new THREE.Vector3(-1.8, 1, 0.6),
        new THREE.Vector3(1.6, -4, -0.8), new THREE.Vector3(-0.8, -9, 0.4), new THREE.Vector3(0.6, -13, 0),
      ]);
      const nodes = [];
      for (let i = 0; i < N; i++) {
        const t = i / (N - 1);
        const p = spine.getPointAt(t);
        const sev = severity[Math.min(2, Math.floor(t * 3))];
        const r = 0.75 + Math.sin(t * Math.PI) * 0.55;
        const disc = new THREE.Mesh(new THREE.CylinderGeometry(r * 1.1, r * 1.1, r * 0.6, 22), metal);
        disc.position.copy(p); disc.lookAt(spine.getPointAt(Math.min(1, t + 0.02))); disc.rotateX(Math.PI / 2);
        g.add(disc);
        const node = new THREE.Mesh(new THREE.IcosahedronGeometry(r * 0.62, 1),
          new THREE.MeshStandardMaterial({ color: 0x0a0a0a, emissive: sev, emissiveIntensity: 0.9, metalness: 0.3, roughness: 0.4 }));
        node.position.copy(p).add(new THREE.Vector3(r * 1.5, 0, 0));
        g.add(node); nodes.push(node);
        if (i > 0) {
          const a = nodes[i - 1].position, b = node.position;
          const mid = a.clone().lerp(b, 0.5).add(new THREE.Vector3(1.2, 0, 0.6));
          const link = new THREE.Line(
            new THREE.BufferGeometry().setFromPoints(new THREE.QuadraticBezierCurve3(a, mid, b).getPoints(16)),
            new THREE.LineBasicMaterial({ color: col, transparent: true, opacity: 0.85 }));
          g.add(link);
        }
      }
      const halo = new THREE.Mesh(new THREE.TorusGeometry(10, 0.08, 8, 90), new THREE.MeshBasicMaterial({ color: col, transparent: true, opacity: 0.35 }));
      halo.rotation.x = Math.PI / 2.4; g.add(halo);
      g.scale.setScalar(0.72); // the 24-unit spine fits a 58° FOV at ~20 units
      viewport = nodes[Math.floor(N / 2)];
  ...
  function buildCreatures(pathPts, col, arche) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // assemble (product): keep the drifting facets small so they read as
      // particles, not as a second hero competing with the subject.
      if (arche === 'assemble') { mesh.scale.setScalar(0.4); mesh.material.opacity = 0.6; }
